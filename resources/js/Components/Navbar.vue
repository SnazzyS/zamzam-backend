    ...
    <nav class="sticky top-0 z-50 border-b border-slate-200 bg-white/95 backdrop-blur">
        <div class="mx-auto flex h-16 max-w-[1400px] items-center gap-2 px-6">
            ...
            <div class="ml-4 flex items-center">
                <svg class="h-7 w-7 text-slate-900" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div class="mr-2 flex items-center gap-4">
                ...
                <div class="flex h-9 w-9 items-center justify-center rounded-full bg-slate-900 transition hover:opacity-90">
    ...
    <div v-if="showCreateModal" class="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm" @click.self="closeCreateModal">
        <div class="w-full max-w-lg rounded-2xl bg-white p-6 shadow-xl">
            ...
            <form @submit.prevent="submitCreate" class="space-y-5">
                <div class="space-y-2">
                    <label for="create-name" class="text-xs font-semibold text-slate-500 text-right">ދަތުރުގެ ނަން</label>
                    <input
                        id="create-name"
                        type="text"
                        ...
                        :class="createForm.errors.name ? 'border-red-500 focus:border-red-500 focus:ring-red-200' : 'focus:border-slate-400'"
                        required
                    >
                ...
                <div class="space-y-2">
                    <label for="create-departure-date" class="text-xs font-semibold text-slate-500 text-right">ފުރާ ތާރީޚު</label>
                    <input
                        ...
                        :class="createForm.errors.departure_date ? 'border-red-500 focus:border-red-500 focus:ring-red-200' : 'focus:border-slate-400'"
                        required
                    >
                ...
                <div class="space-y-2">
                    <label for="create-price" class="text-xs font-semibold text-slate-500 text-right">އަގު (MVR)</label>
                    <input
                        id="create-price"
                        type="number"
                        ...
                        dir="ltr"
                        class="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 transition focus:outline-none focus:ring-2 focus:ring-slate-200"
                        :class="createForm.errors.price ? 'border-red-500 focus:border-red-500 focus:ring-red-200' : 'focus:border-slate-400'"
                        required
                    >
                ...
                <div class="flex items-center justify-end gap-3 pt-2">
                    <button type="button" @click="closeCreateModal" class="rounded-lg border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">ކެންސަލް</button>
                    <button type="submit" class="rounded-lg bg-slate-900 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-70" :disabled="createForm.processing">
