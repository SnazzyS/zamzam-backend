        ...
        <div class="rounded-xl border border-slate-200 bg-white">
            ...
            <div class="p-5">
                <div v-if="filteredTrips.length > 0" class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    ...
                    >
                        <Link
                            ...
                            class="block p-5"
                        >
                            <div class="mb-3 flex items-start justify-between pr-10">
                                <h3 class="font-semibold text-slate-900 group-hover:text-violet-600">{{ trip.name }}</h3>
                                <span :class="['inline-flex items-center rounded-full px-2 py-1 text-xs font-medium ring-1 ring-inset', getStatusBadge(trip.status)]">
                                    {{ trip.status }}
                                </span>
                            </div>
                        ...
                        <button
                            type="button"
                            class="absolute right-4 top-4 inline-flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-600 shadow-sm transition hover:border-violet-200 hover:text-violet-600"
                ...
                <div v-else class="flex flex-col items-center justify-center py-12 text-center">
                    ...
                    <p class="font-medium text-slate-900">No trips found</p>
                    <p class="mt-1 text-sm text-slate-500">
                        {{ activeFilter === 'active' ? 'There are no active trips at the moment.' : 'No trips have been created yet.' }}
                    </p>
                </div>
            </div>
        </div>
